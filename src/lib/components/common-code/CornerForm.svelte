<script lang="ts">
  ...
  import {
    createCorner,
    updateCorner,
    type CreateCornerDto,
    type UpdateCornerDto
  } from '$lib/api/corners';
  import { Button } from '$lib/components/ui/button/index.js';
  import type { MPW_CORNER } from '$lib/schemas/corner.schema';

  interface Props {
    editMode?: boolean;
    initialData?: MPW_CORNER | null;
    onSuccess?: () => void;
    onCancel?: () => void;
  }
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (editMode && initialData) {
        // 수정 모드
        const updateData: UpdateCornerDto = {
          name: name.trim(),
          avail
        };
        await updateCorner(initialData.code, updateData);
      } else {
        // 추가 모드
        const createData: CreateCornerDto = {
          name: name.trim(),
          avail
        };
        await createCorner(createData);
      }
      ...
      error = editMode ? 'Corner 수정에 실패했습니다.' : 'Corner 추가에 실패했습니다.';
      console.error(e);
    } finally {
      loading = false;
    }
