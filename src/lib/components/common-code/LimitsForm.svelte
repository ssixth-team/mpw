<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    if (minValue >= maxValue) {
      error = 'Min Value는 Max Value보다 작아야 합니다.';
      return;
    }
    ...
    try {
      if (editMode && initialData) {
        // 수정 모드
        const updateData: UpdateLimitsDto = {
          name: name.trim(),
          minValue,
          maxValue,
          avail
        };
        await updateLimits(initialData.code, updateData);
      } else {
        // 추가 모드
        const createData: CreateLimitsDto = {
          name: name.trim(),
          minValue,
          maxValue,
          avail
        };
        await createLimits(createData);
      }
      ...
      error = editMode ? 'Limits 수정에 실패했습니다.' : 'Limits 추가에 실패했습니다.';
      console.error(e);
    } finally {
      loading = false;
    }
...
<form onsubmit={handleSubmit} class="space-y-4">
  ...
  <div class="form-row">
    <div class="form-group">
      <label for="minValue">Min Value <span class="required">*</span></label>
      <input
        id="minValue"
        type="number"
        bind:value={minValue}
